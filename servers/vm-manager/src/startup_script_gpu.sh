...
# Run docker image
# current segmentation model cannot be initialized as a subprocess, hence needs to be run as a non-concurrent worker (pool=solo)
docker run -d --name $QUEUE --gpus all -v /data:/data -v /flex_pipeline_data_accept:/flex_pipeline_data_accept -e VM_NAME=$(hostname) -e VM_NAME_PREFIX=$VM_NAME_PREFIX \
...
crontab -r
crontab -l > mycron
cmd="*/1 * * * * sudo bash /tmp/shutdown_script.sh"
